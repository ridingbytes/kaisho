"""Markdown backend using JSON files as storage.

Each domain gets its own JSON file inside a configurable directory
(default: data/markdown/). Despite the name "markdown", the backend
stores data as JSON for simpler parsing and writing.
...
def _period_range(period: str) -> tuple[date, date]:
    """Return (start_date, end_date) for 'today'/'week'/'month'."""
    today = date.today()
    if period == "today":
        return today, today
    if period == "week":
...
class MarkdownClockBackend(ClockBackend):
    ...
    def _enrich(self, entry: dict) -> dict:
        ...
        entry["duration_minutes"] = self._duration_minutes(entry)
        return entry
    ...
    def get_summary(self, period="month") -> list[dict]:
        ...
        for e in entries:
            ...
            totals[cust] = (
                totals.get(cust, 0) + e["duration_minutes"]
            )
        return [
            {
                "customer": cust,
    ...
    def start(
        ...
        if self.get_active() is not None:
            raise ValueError("A clock entry is already running")
        entries = _read_json(self._clocks_file)
        entry = {
            "customer": customer,
    ...
    def delete_entry(self, start_iso) -> bool:
        entries = _read_json(self._clocks_file)
        new = [
            e for e in entries if e.get("start") != start_iso
        ]
...
class MarkdownCustomerBackend(CustomerBackend):
    ...
    def _used_hours(self, customer_name: str) -> float:
        """Sum booked hours from clocks for *customer_name*."""
        entries = _read_json(self._clocks_file)
        total_min = 0
    ...
    def update_customer(self, name, updates) -> dict | None:
        ...
        for c in custs:
            if c.get("name", "").lower() == low:
                for key in ("name", "status", "kontingent",
                            "repo"):
                    if key in updates:
                        c[key] = updates[key]
                _write_json(self._customers_file, custs)
    ...
    def list_contracts(self, name) -> list[dict]:
        ...
        for c in custs:
            if c.get("name", "").lower() == low:
                ...
                return [
                    self._enrich_contract(c["name"], con)
    ...
    def delete_contract(self, name, contract_name) -> bool:
        custs = _read_json(self._customers_file)
        low = name.lower()
        for c in custs:
            if c.get("name", "").lower() != low:
...
class MarkdownNotesBackend(NotesBackend):
    ...
    def promote_to_task(self, note_id, tasks, customer) -> dict:
        notes = _read_json(self._notes_file)
        target = None
        remaining = []
        for note in notes:
...
def make_markdown_backend(cfg) -> tuple[
    ...
    """Build markdown/JSON backends from config paths."""
    ...
    clocks = MarkdownClockBackend(md_dir / "clocks.json")
    inbox = MarkdownInboxBackend(md_dir / "inbox.json")
    cust = MarkdownCustomerBackend(
        md_dir / "customers.json",
        md_dir / "clocks.json",
    )
    notes = MarkdownNotesBackend(md_dir / "notes.json")
