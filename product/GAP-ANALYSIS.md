...
| Auth (login, register, multi-user) | -- | `/api/auth/*` | LoginPage |
...
| Multi-profile per user | Working |
| Pluggable backends (org, markdown, json) | Working |
| WebSocket real-time updates | Working |
| File watcher (auto-reload on external changes) | Working |
| CORS (localhost only) | Working |
| Rate limiting (auth endpoints) | Working |
