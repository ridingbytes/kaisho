...
~/develop/kaisho/
  ...
  data/                          # Runtime (gitignored)
    kaisho.db             # SQLite: Knowledge FTS,
...
Die CLI liest die gleiche Config wie die API (`kaisho/config.py`).
...
Python 3.12.12 via pyenv. Virtualenv: kaisho.
...
### Indexing
SQLite FTS5 in data/kaisho.db (gleiche DB wie Communications):
...
3. SQLite Setup (kaisho.db: Communications, FTS, Cron)
...
| Eine SQLite-Datei         | Alles in kaisho.db     | Kein DB-Wildwuchs, ein Backup   |
...
## Installation
git clone git@github.com:ridingbytes/kaisho.git
cd kaisho
pip install -e .
...
uvicorn kaisho.api.app:app --reload  # http://localhost:8000
```
...
# /etc/systemd/system/kaisho.service
[Unit]
...
Type=simple
User=ramon
WorkingDirectory=/home/ramon/develop/kaisho
ExecStart=/home/ramon/develop/kaisho/.venv/bin/uvicorn \
    kaisho.api.app:app --host 0.0.0.0 --port 8000
Restart=always
...
## launchd (macOS)
# ~/Library/LaunchAgents/com.ridingbytes.kaisho.plist
