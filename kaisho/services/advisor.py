...
_BASE_SYSTEM_PROMPT = (
    ...
    "capacity / 'over budget' / 'what should I focus on' "
    "questions. Those contracts are historical and have no "
    "remaining capacity. Only ``state == 'active'`` contributes "
    "to the customer's live budget. If only ``invoiced`` / "
    "``ended`` contracts exist, the customer currently has no "
    "active contract -- say so explicitly.\n"
    "5a. INTENT -> TOOL MAPPING. The following intents have a "
    "dedicated tool. Use the dedicated tool, NOT execute_cli, NOT a "
    "hallucinated CLI command:\n"
    "    - calendar / meetings / schedule / 'what's next week'\n"
    "      / 'what should I focus on' / 'plan my week'\n"
    "      / 'what's coming up' / any planning question\n"
    "      -> list_calendar_events  (covers CalDAV + Google;\n"
    "         do NOT run `kai calendar` -- there is no such CLI)\n"
    "      Planning questions ALWAYS need both the task list AND\n"
    "      the calendar -- meetings shape the time available.\n"
    "    - one specific calendar event by id\n"
    "      -> get_calendar_event\n"
    "    - knowledge base lookup / docs / how does X work\n"
    "      -> search_knowledge, then read_knowledge_file\n"
    "    - list clock entries\n"
    "      -> list_clock_entries\n"
    "    - list / show tasks / inbox / notes / customers\n"
    "      -> list_tasks / list_inbox / list_notes / list_customers\n"
    "Only fall back to execute_cli if NO dedicated tool fits. NEVER "
    "claim a feature is missing without first trying the dedicated "
    "tool above. If a dedicated tool returns an error, report the "
    "error -- do NOT pivot to 'feature unavailable'.\n"
    ...
    if provider == "kaisho":
        ...
        cli_tool = [{
            ...
            "function": {
                "name": "execute_cli",
                "description": (
                    "Run a kai CLI command -- ONLY as a "
                    "fallback when no dedicated tool fits. "
                    "Examples: task add CUSTOMER \"Title\", "
                    "clock start CUSTOMER, "
                    "clock stop, "
                    "clock book 2h CUSTOMER \"desc\", "
                    "note add \"Title\". "
                    "DO NOT use this for calendar queries -- "
                    "use list_calendar_events instead "
                    "(there is no `kai calendar` command). "
                    "DO NOT use this for KB / tasks / inbox / "
                    "notes / customers listings -- those have "
